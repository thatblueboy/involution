...
class _DenseLayer(nn.Module):
    def __init__(
        self, num_input_features: int, growth_rate: int, bn_size: int, drop_rate: float, memory_efficient: bool = False, has_involution = False) -> None:
        super().__init__()
        self.has_involution = has_involution
        ...
        if has_involution:
            self.involution = Involution(bn_size * growth_rate, kernel_size = 3, stride=1 )
            self.involve_relu = nn.ReLU(inplace=True)
            self.involve_norm = nn.BatchNorm2d(bn_size * growth_rate)

        self.conv2 = nn.Conv2d(bn_size * growth_rate, growth_rate, kernel_size=1, stride=1, bias=False) if has_involution else nn.Conv2d(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False)
